...
void iterate(byte * hash1, byte * hash2, char *str, int idx, int len, int *ok) {
    ...
    /*
     *  TODO: Maybe idx and len should be local to each thread
     * */
...
# pragma omp parallel for
            for (c = 0; c < strlen(letters); ++c) {
                ...
                /* locals initiation */
                char nstr[MAX+1];
                byte nhash[MD5_DIGEST_LENGTH];

                memset(nstr, 0, MAX+1);
                memset(nhash, 0, MD5_DIGEST_LENGTH);
                nstr[idx] = letters[c];
                // Recursive call
                iterate(hash1, nhash, nstr, idx + 1, len, ok);
            }
        }
    } else {
        if (idx > 0) {
